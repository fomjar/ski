...
insert into tbl_cmd_map values('taobao-order-proc-new', 'st', 3, "select rownum, c_toid, c_fpid, c_tuid from (select @rounum := @rownum + 1 as rownum, t_time, c_toid, c_fpid, c_tuid from tbl_order_taobao t1, (select @rownum := 0) t2) t3 order by t3.t_time;");
