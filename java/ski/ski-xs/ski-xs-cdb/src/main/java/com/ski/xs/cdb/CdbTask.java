...
public class CdbTask implements FjServerTask {
    ...
    private static void generateSql(Instruction inst) {
        logger.debug(String.format("inst(0x%08X) sql-ori: %s", inst.inst, inst.sql_ori));
        ...
        logger.debug(String.format("inst(0x%08X) sql-use: %s", inst.inst, sql_use));
    }
